...
# 配置 MoviePy 使用正确的 ffmpeg
os.environ['FFMPEG_BINARY'] = subprocess.run(['which', 'ffmpeg'], capture_output=True, text=True).stdout.strip()
from moviepy.video.io.ImageSequenceClip import ImageSequenceClip

# ========================= 配置区 =========================
# repo_id = "hhuangbt/Folding_50_12_13"

original_dir = "/home/agilex/cobot_magic/collect_data/data/folding_clothes_250/"
out_dir = "/home/agilex/cobot_magic/collect_data/data/folding_clothes_250_lerobot_av1/"

TEST_ONLY_FIRST_N = None  # 设为 None 运行全部


# VIDEO_CODEC = 'libx264'  #h264
VIDEO_CODEC = 'libsvtav1' # av1
    ...
    try:
        with h5py.File(episode_path, 'r') as f:
            ...
            episode_video_info = {
                ...
                "tasks": [task_index],  # list[int]
                "length": n,
                "data/chunk_index": data_chunk_idx,
                "data/file_index": data_file_idx,
                "dataset_from_index": episode_data_from,
            ...
            for cam in cameras:
                ...
                #  图像统计
                if len(frames) > 0:
                    frames_array = np.stack(frames).astype(np.float32) / 255.0  # (n, 256, 256, 3) -> [0,1]
                    pixels = frames_array.reshape(-1, 3)  # (n*256*256, 3)

                    percs = [1, 10, 50, 90, 99]
                    q = np.percentile(pixels, percs, axis=0)  # (5, 3)

                    all_image_stats[cam]["min"].append(pixels.min(axis=0).tolist())
                    all_image_stats[cam]["max"].append(pixels.max(axis=0).tolist())
                    all_image_stats[cam]["mean"].append(pixels.mean(axis=0).tolist())
                    all_image_stats[cam]["std"].append(pixels.std(axis=0).tolist())
                    all_image_stats[cam]["count"].append([len(frames)])
                    all_image_stats[cam]["q01"].append(q[0].tolist())
                    all_image_stats[cam]["q10"].append(q[1].tolist())
                    all_image_stats[cam]["q50"].append(q[2].tolist())
                    all_image_stats[cam]["q90"].append(q[3].tolist())
                    all_image_stats[cam]["q99"].append(q[4].tolist())





                episode_video_info[f"videos/observation.images.{cam}/chunk_index"] = video_chunk_idx[cam]
                episode_video_info[f"videos/observation.images.{cam}/file_index"] = video_file_idx[cam]
                episode_video_info[f"videos/observation.images.{cam}/from_timestamp"] = float(time_stamp[0] - episode_start_time) if n > 0 else 0.0
                episode_video_info[f"videos/observation.images.{cam}/to_timestamp"] = float(time_stamp[-1] - episode_start_time) if n > 1 else 0.0

            episode_meta_rows.append(episode_video_info)

            # 为每个 episode 添加图像 stats 列
            for ep_idx in range(len(episode_meta_rows)):
                row = episode_meta_rows[ep_idx]
                for cam in cameras:
                    if len(all_image_stats[cam]["min"]) > ep_idx:  # 确保有数据
                        for stat_key in ["min", "max", "mean", "std", "count", "q01", "q10", "q50", "q90", "q99"]:
                            values = all_image_stats[cam][stat_key][ep_idx]
                            nested = [[[v] for v in values]]  # [[[r], [g], [b]]]
                            row[f"stats/observation.images.{cam}/{stat_key}"] = nested
