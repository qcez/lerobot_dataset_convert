...
original_dir = "/home/zwangou/HDF5toLerobot/original_data"
out_dir = "/home/zwangou/HDF5toLerobot/folding_clothes_251_lerobot/"
    ...
    try:
        with h5py.File(episode_path, 'r') as f:
            ...
            for cam in cameras:
                ...
                # libsvtav1 使用数字 preset (0-13)，6 是平衡速度和质量的选择
                if VIDEO_CODEC == 'libsvtav1':
                    clip.write_videofile(temp_video_path, codec=VIDEO_CODEC, audio=False, logger=None, 
                                       ffmpeg_params=['-pix_fmt', 'yuv420p', '-preset', 8])
                else:
                    clip.write_videofile(temp_video_path, codec=VIDEO_CODEC, audio=False, logger=None, 
                                       ffmpeg_params=['-strict', '-2', '-pix_fmt', 'yuv420p'])
...
video_info = {
    "video.height": 480,
    "video.width": 640,
    "video.codec": "av1" if VIDEO_CODEC in ['libaom-av1', 'libsvtav1'] else "h264",
